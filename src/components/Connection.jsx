...
const Connection = () => {
  ...
  return (
    ...
    // </div>
    <div className="flex justify-center items-center min-h-screen mt-5 mb-10 pb-6">
  <div className="space-y-6 px-6 sm:px-10 w-full max-w-4xl">
    <h1 className="text-center text-2xl font-bold">Connections</h1>
    {error && <p className="text-red-600">{error}</p>}
    {connections.map((connection) => {
      const { _id, firstName, lastName, age, about, skills, gender, photoURL } = connection;
      ...
      return (
        <div
          key={_id}
          className="card card-side bg-base-300 shadow-md flex flex-col sm:flex-row items-center sm:items-start px-5 py-4 w-full rounded-lg mb-6"
        >
          {/* Profile Image */}
          <figure className="w-32 h-32 flex-shrink-0">
            <img
          ...
          <div className="card-body w-full">
            ...
            <p className="text-gray-400 text-center sm:text-left"><strong>Age:</strong> {age}</p>
            <p className="text-gray-400 text-center sm:text-left"><strong>Gender:</strong> {gender}</p>
